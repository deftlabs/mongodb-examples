...
public final class ReplicaSetExample {
    ...
    public void readWriteData() throws Exception {
        ...
        final List<ServerAddress> replicaSetSeeds = new ArrayList<ServerAddress>();

        for (final int port : _ports) {
            if (port == 27020) continue;
            replicaSetSeeds.add(new DBAddress("127.0.0.1", port, "test"));
        }

        final Mongo mongo = new Mongo(replicaSetSeeds);
        mongo.slaveOk();

        mongo.getDB("test").getCollection("replicaSet").remove(new BasicDBObject());

        // Give some time to persist and replicate.
        Thread.sleep(3000);

        for (int idx=0; idx < 100; idx++) {
            final BasicDBObject test = new BasicDBObject("name", ("test" + idx));
            mongo.getDB("test").getCollection("replicaSet").insert(test);
        }
        ...
        for (int idx=0; idx < 100; idx++) {
            assertEquals(100, mongo.getDB("test").getCollection("replicaSet").count());
            assertNotNull(mongo.getDB("test").getCollection("replicaSet").find(new BasicDBObject("name", "test99")));
        }
    }
    */
}
